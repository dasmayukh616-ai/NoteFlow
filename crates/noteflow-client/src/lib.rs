...
use noteflow_types::{
    AIMessage, CalendarEvent, CalendarIntegration, Page, UsageStats, User, Workspace,
};
...
impl ParityApiClient {
    pub fn new(config: ClientConfig) -> Self {
        Self {
            config,
            #[cfg(not(target_arch = "wasm32"))]
            native_http: reqwest::Client::new(),
        }
    }
    ...
    T: DeserializeOwned,
{
    ...
    match envelope {
        ...
        Ok(ApiEnvelope::Ok { ok: false, .. }) | Ok(ApiEnvelope::Err { ok: true, .. }) => {
            Err(ClientError::InvalidResponse("invalid parity response envelope".into()))
        }
